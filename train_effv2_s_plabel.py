...
label_path = "/datadisk/kg/seti/input/plabel.csv"
...
def train_transforms():
    return A.Compose([
        ...
        A.Cutout(num_holes=5, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.5),
        ToTensorV2(),
    ])
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/seti-{model_arch}-fold{fold}-{img_size}-plabel_auc.pth'))
...
def main_loop(resume):
    ...
    for fold in range(cfg['fold_num']):
        ...
        if resume:
            ...
            checkpoint = torch.load(f"checkpoint/seti-{cfg['model_arch']}-fold{fold}-{cfg['img_size']}-plabel_auc.pth")
