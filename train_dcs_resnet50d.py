...
cfg = {
    'fold_num': 5,
    'seed': 2021,
    'model_arch': 'resnet50d',
    'img_size': 768,
    'epochs': 8,
    'train_batch_size': 128,
    'val_batch_size': 128*2,
    'T_0': 1,
    'T_mul': 2,
    'lr': 3e-4,
    'min_lr': 6e-5,
...
def train_transforms():
    return A.Compose([
        ...
        A.Cutout(num_holes=10, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.3),
...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/seti-{model_arch}-fold{fold}-plabel-auc.pth'))
...
def main_loop(resume):
    ...
    for fold in range(cfg['fold_num']):
        ...
        if resume:
            ...
            checkpoint = torch.load(f"checkpoint/seti-{cfg['model_arch']}-fold{fold}-plabel-auc.pth")
