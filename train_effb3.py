...
def save_model(net, fold, auc=.0, loss=.0):
    ...
    torch.save(state, os.path.join(project_path, f'checkpoint/seti-{model_arch}-fold{fold}-auc.pth'))
...
def main_loop(resume):
    ...
    for fold in range(cfg['fold_num']):
        ...
        if resume:
            ...
            checkpoint = torch.load(f"checkpoint/seti-{cfg['model_arch']}-fold{fold}-auc.pth")['net']
