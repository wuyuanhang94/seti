...
def train_transforms():
    return A.Compose([
        ...
        A.Cutout(num_holes=10, max_h_size=int(0.08*cfg['img_size']), max_w_size=int(0.08*cfg['img_size']), p=0.5),
...
def main_loop(resume):
    ...
    for fold in range(cfg['fold_num']):
        ...
        if resume:
            ...
            checkpoint = torch.load(f"checkpoint/seti-{cfg['model_arch']}-fold{fold}-auc.pth")
